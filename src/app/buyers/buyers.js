(function() {
    ...
    BuyerConfig.$inject = ["$stateProvider"];
    BuyerController.$inject = ["$state", "$ocMedia", "BuyerList", "OrderCloudParameters", "OrderCloud", "Parameters"];
    BuyerEditController.$inject = ["$exceptionHandler", "$state", "$i18next", "SelectedBuyer", "OrderCloud", "toastr", "$q", "LabeledSpecs", "$scope", "option_list", "Audit"];
    BuyerCreateController.$inject = ["$exceptionHandler", "$state", "$i18next", "OrderCloud", "toastr", "option_list", "Audit"];
    BuyerProductsController.$inject = ["$scope", "$exceptionHandler", "$i18next", "$state", "SelectedBuyer", "ProductList", "Assignments", "FullOrderCloud", "OrderCloud", "toastr", "Underscore", "$q", "$stateParams", "ProductAssignments", "Paging", "ProductGroups", "dragulaService", "Helpers", "option_list", "Audit"];
    angular.module('productManager').config(BuyerConfig).controller('BuyerCtrl', BuyerController).controller('BuyerEditCtrl', BuyerEditController).controller('BuyerCreateCtrl', BuyerCreateController).controller('BuyerProductsCtrl', BuyerProductsController);

    function BuyerConfig($stateProvider) {
        'use strict';

        $stateProvider.state('buyers', {
            parent: 'base',
            ...
            resolve: {
                ...
                ProductGroups: ["ResourceService", function ProductGroups(ResourceService) {
                    return ResourceService.ProductGroups();
                }],
                ProductList: ["OrderCloud", "Assignments", "Helpers", function ProductList(OrderCloud, Assignments, Helpers) {
                    var ids = Helpers.getFilterKeys(Assignments, true, "ProductID", "ID");

                    return OrderCloud.Products.List(null, 1, 100, null, null, {
                        'ID': ids
                    });
                }],
                ProductAssignments: ["Assignments", "OrderCloud", "Helpers", function ProductAssignments(Assignments, OrderCloud, Helpers) {
                    var ids = Helpers.getFilterKeys(Assignments, false, "ProductID", "ID");
    ...
    function BuyerProductsController($scope, $exceptionHandler, $i18next, $state, SelectedBuyer, ProductList, Assignments, FullOrderCloud, OrderCloud, toastr, Underscore, $q, $stateParams, ProductAssignments, Paging, ProductGroups, dragulaService, Helpers, option_list, Audit) {
        var vm = this;
        vm.buyer = SelectedBuyer;
        vm.buyerName = SelectedBuyer.Name;
        vm.list = ProductList;
        ...
        vm.research = function() {
            ...
            var idList = Helpers.getFilterKeys(Assignments, false, "ProductID", "ID");
            var exList = Helpers.getFilterKeys(Assignments, true, "ProductID", "ID");
            ...
            if (vm.listFilter) {
                ...
                OrderCloud.Products.List(null, 1, 100, null, null, {
                    'ID': exList
                }).then(function(data) {
                    vm.list = data;
                });
        ...
        vm.assign = function(product) {
            ...
            OrderCloud.Products.SaveAssignment(assignment).then(function() {
                ...
                Audit.log($i18next.t("Channel Product Assignment"), product.Name + " > " + SelectedBuyer.Name, [{
                    "Type": "Channel",
                    "TargetID": SelectedBuyer.ID,
                    "NewState": product.ID
                }], 'CREATE');
        ...
        function ListPagingFunction() {
            var exList = Helpers.getFilterKeys(Assignments, true, "ProductID", "ID");
            return Paging.filteredPaging(vm.list, 'Products', vm.searchFilter, {
                'ID': exList
            });
        }
        ...
        function AssignmentPagingFunction() {
            var idList = Helpers.getFilterKeys(Assignments, false, "ProductID", "ID");
            return Paging.filteredPaging(vm.assignments, 'Products', vm.searchFilter, {
                'ID': idList
            });
        }
    };
