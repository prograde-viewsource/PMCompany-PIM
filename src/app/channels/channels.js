...
angular.module('productManager')
    ...
    .controller('ChannelsCtrl', ChannelsController)
    .controller('ChannelEditCtrl', ChannelEditController)
    .controller('ChannelCreateCtrl', ChannelCreateController)
    .controller('ChannelProductsCtrl', ChannelProductsController)
...
function ChannelConfig($stateProvider) {
    ...
    // List State
        .state('channels', {
            ...
            data: { componentName: 'Channels', OrderIndex: 5 },
            resolve: {
                Parameters: function($stateParams, OrderCloudParameters) {
                    return OrderCloudParameters.Get($stateParams);
                },
                BuyerList: function(OrderCloud, Parameters) {
                    let sort = Parameters.sortBy ? (Parameters.sortBy.indexOf('!') === 0 ? Parameters.sortBy.split('!')[1] : Parameters.sortBy) : "Name";
        ...
        .state('channels.products', {
            ...
            resolve: {
                Assignments: function(FullOrderCloud, $stateParams) {
                    return FullOrderCloud.Products.List(null, 1, 100, null, null, { 'catalogID': $stateParams.buyerid });
                ...
                ProductList: function(OrderCloud, FullOrderCloud, Helpers, $q, $stateParams, Assignments) {
                ...
                ProductAssignments: function($stateParams, FullOrderCloud, OrderCloud, Helpers, $q) {
...
function GetProductList(OrderCloud, FullOrderCloud, Helpers, $q, buyerid, searchFilter = null, paramObject = null) {
    let ret = []
    let q = [];
    let uberQ = $q.defer();
    FullOrderCloud.Products.ListAssignments(null, null, null, null, null, null, null, buyerid).then(function(Assignments) {
        q.push(function() {
            let ids = Helpers.getFilterKeys(Assignments, false, "ProductID", "ID").split("|");
            try {
            ...
            OrderCloud.Products.List(searchFilter, 1, 100, null, null, paramObject).then(function(data) {
                ...
                let pNumber = 1;
                if (data.Meta.TotalPages == 1) {
                    _.delay(function() { uberQ.resolve(data); }, 250);
                }
...
function GetProductAssignmentLists(OrderCloud, FullOrderCloud, Helpers, $q, buyerid, searchFilter = null, paramObject = {}) {
    let uberQ = $q.defer();
    FullOrderCloud.Products.ListAssignments(null, null, null, null, null, null, null, buyerid).then(function(Assignments) {
        let base = { Items: [], Meta: { TotalCount: Assignments.Items.length, ItemRange: [Assignments.Items.length, Assignments.Items.length] } };
        let q = [];
        let lists = _.groupBy(Assignments.Items, function(element, index) {
            return Math.floor(index / 50);
        });
        angular.forEach(lists, function(list) {
            q.push(function() {
                let tL = { "Items": list };
                let ids = Helpers.getFilterKeys(tL, false, "ProductID", "ID");
                paramObject.ID = ids;
                OrderCloud.Products.List(searchFilter, 1, 100, null, null, paramObject).then(function(data) {
        ...
        $q.all(q).then(function() {
            _.delay(function() { uberQ.resolve(base); }, 500);
        });
    });
    return uberQ.promise;
}

function ChannelProductsController($http, $rootScope, $scope, $exceptionHandler, $i18next, $state, SelectedBuyer, ProductList, Assignments, FullOrderCloud, OrderCloud,
    toastr, Underscore, $q, $stateParams, ProductAssignments, Paging, ProductGroups, dragulaService, Helpers, option_list, Audit, $uibModal) {
    "use strict";
    var vm = this;
    vm.buyer = SelectedBuyer;
    vm.buyerName = SelectedBuyer.Name;
    ...
    vm.research = _.debounce(() => {
        ...
        if (vm.brandFilter === null)
            vm.brandFilter = undefined;

        if (vm.searchFilter === null)
            vm.searchFilter = undefined;

        if (vm.statusFilter && vm.statusFilter !== '') { vm.paramObject["xp.Status"] = vm.statusFilter; }
        ...
        if (vm.brandFilter && vm.brandFilter !== '') { vm.paramObject["xp.Brand-Name"] = vm.brandFilter; }
        ...
        if (vm.listFilter) {
            ...
            OrderCloud.Products.List(keywords, 1, 100, null, searchby, vm.paramObject).then(function(_data) {
                ...
                if (!vm.assignedFilter) { $rootScope.$emit('$stateChangeSuccess', $state); };
            });
        } else {
        ...
        if (vm.assignedFilter) {
            ...
            OrderCloud.Products.List(keywords, 1, 100, null, searchby, vm.paramObject).then(function(data) {
                vm.assignments = data;
                $rootScope.$emit('$stateChangeSuccess', $state);
            });
        } else {
            OrderCloud.Products.List(null, 1, 100, null, null, { catalogID: $stateParams.buyerid }).then(function(data) {
                vm.assignments = data;
                $rootScope.$emit('$stateChangeSuccess', $state);
            });
        }
    ...
    vm.listPagingFunction = function() {
        ...
        if (vm.list.Meta.TotalCount - vm.assignments.Meta.TotalCount < expectedCount)
            expectedCount = vm.list.Meta.TotalCount - vm.assignments.Meta.TotalCount;
